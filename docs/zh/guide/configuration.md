...
静态检查包括路由编译（host/path 正则）、HTTPS/证书结构，以及 **`type: service`** 下的每条 backend：

- **`service` 与 `redirect`** 不能在同一 backend 上同时生效（`service.name` 非空且配置了 `redirect.url` 会被拒绝）。
- **仅 `redirect`** 的后端可以不写 `service`。

报错中会标明规则下标、配置的 host 模式以及路由 path：**`rules[N] host="..." path="..."`**。规则级 backend 使用 **`path="/"`**；子路径 backend 使用 **`paths[].path`** 中的配置字符串（若为空则退化为 `paths[index]`）。
