...
- **Host entries**: Plain string or `{ host, window }` via `service.MaintenanceHostList.UnmarshalYAML` (`core/service/maintenance_hosts.go`). **`window.start` and `window.end` are required** on every `maintenance.hosts[]` entry and on **`service.maintenance.window`** when `enabled: true` and `scope: all`.
- **Runtime**: `compiledMaintenanceHostList.MatchActive(hostname, now)` — host pattern match **and** per-entry window; rule `scope: all` uses `service.maintenance.window`. Global and route hits merge bypass; route `title` / `subtitle` / `retry_after` override global when the route maintenance triggered.
- **Logs**: Access log `maintenance_block=1` on 503 maintenance responses; maintenance responses send configurable **`response_header`** (default **`X-Ingress-Maintenance: 1`**). When the matched host entry has **`window`**, also **`X-Ingress-Maintenance-From`** / **`X-Ingress-Maintenance-Until`** (RFC3339 from `window.start` / `window.end`).
