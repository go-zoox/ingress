...
func (s *Ingress) ConfigPath() string {
	return s.cfg.IngressConfigPath
}
...
// ReloadReady reports whether config reload is available.
...
func (s *Ingress) Reload() error {
	...
	if s.cfg.ReloadFn != nil {
		return s.cfg.ReloadFn()
	}
	pid, err := s.readPID()
	if err != nil {
		return fmt.Errorf("%w — start ingress: ingress run -c %s --pid-file %s", err, s.ConfigPath(), s.cfg.PidFile)
	}
	proc, err := os.FindProcess(pid)
	if err != nil {
		return err
